...
let m =
  "Lorem ipsum dolor sit amet consecte wtur adipisicing brbvweruv ewruv ewrugv eruvreuoecrxh hre byugvuyxbuery xeruyffgereuqionde d3iud uudqoudnzqnmv bvcv bunrhf x8fiurfi4hf8347xf hq3indo3id iquznqf ryzf380fz q34fn3q4inf43kcfnpi fx80hf0q38b8";
...
class Hasher {
  ...
  addPaddingPKCS7(length) {
    this.state.message += new Array(length + 1).join(
...
/** @type {number[]} */
const SBOX0 = [
  ,
];
/** @type {number[]} */
const eBOX = [
    ...
    for (let r = 1; r <= this.options.rounds; r++) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        for (let t = 0, s = 56, j = 0; t < 8; t++, s -= 8, j = s < 32 ? 1 : 0) {
          L[i * 2] ^=
            this.C[t][((K[((i - t) & 7) * 2 + j] >>> s % 32) & 0xff) * 2];
          L[i * 2 + 1] ^=
            this.C[t][((K[((i - t) & 7) * 2 + j] >>> s % 32) & 0xff) * 2 + 1];
        }
      }
      for (let i = 0; i < 16; i++) {
        K[i] = L[i];
      ...
      for (let i = 0; i < 8; i++) {
        ...
        for (let t = 0, s = 56, j = 0; t < 8; t++, s -= 8, j = s < 32 ? 1 : 0) {
          L[i * 2] ^=
            this.C[t][((state[((i - t) & 7) * 2 + j] >>> s % 32) & 0xff) * 2];
          L[i * 2 + 1] ^=
            this.C[t][
              ((state[((i - t) & 7) * 2 + j] >>> s % 32) & 0xff) * 2 + 1
            ];
        }
      }
      for (let i = 0; i < 16; i++) {
...
function fromUtf(message) {
  ...
  for (let i = 0, msgLen = message.length; i < msgLen; i++) {
    ...
    // surrogate pair
      ...
      charCode =
        0x10000 +
        (((charCode & 0x3ff) << 10) | (message.charCodeAt(i) & 0x3ff));
